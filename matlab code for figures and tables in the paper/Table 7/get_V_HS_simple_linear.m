function [v_HS_simple,s_star_simple]=get_V_HS_simple_linear(N,m,r,w,pd0_theta,pd1_theta,pi,theta,p_s)

%get_V_HS_simple_linear generalizes get_V_HS_simple by considering both
%general case and special case with lower computation time.
...
for s_num=0:N*m
    ...
    %
    q_s=zeros(length(a),m*theta+1);
    for t=1:length(a)
        q_s(t,:)=binopdf(0:m*theta,a(t),p_s);
    end
    %V = cell(N,1);
    %for i=1:N
                ...
                for D0=0:(L0_theta-1)  %random emergency patients arriving in period t (they get service from period t+1 onwards)
                    ...
                    for D1=0:(L1_theta-1)  %random inpatients arriving in period t (they get service from period t+1 onwards)
                        ...
                        if (pi(1)+r(1) > pi(2)+r(2)) && (w(1) > w(2))
                            ...
                            q2=min(max(theta*m-D0-(x1+D1),0),x2+k-1);
                            g_final=V_2(X1+D1-q1,X2+k-1-q2)+r(1)*q1+r(2)*q2;
                        elseif (pi(1)+r(1) < pi(2)+r(2)) && (w(1) < w(2))
                            q1=min(max(theta*m-D0-(x2+k-1),0),x1+D1);
                            q2=min(theta*m-D0,x2+k-1);
                            g_final=V_2(X1+D1-q1,X2+k-1-q2)+r(1)*q1+r(2)*q2;
                        else                 
                            for q1=0:min(theta*m-D0,x1+D1) %allocation for inpatients at the beginning of period t+1
                                 for q2=0:min(x2+k-1,theta*m-D0-q1) %allocation for outpatients at the beginning of period t+1
                       ...
                       %outpatients arrive in period t
                                         ...
                                         g=V_2(X1+D1-q1,X2+k-1-q2)+r(1)*q1+r(2)*q2;
                                         if g>g_final
                        ...
                        %%%%%stationary arrival%%%%%
                        V_1(X1,X2)=V_1(X1,X2)+q_s(t+1,k)*pd0_theta(D0+1)*pd1_theta(D1+1)*g_final;
    ...
    if V_1(1,1)>=v_HS_simple  %%%find optimal value and schedule number. Note that the profit is calcuated from period 2 onwards.
        v_HS_simple=V_1(1,1);
        s_star_simple=s_num;     
    end
