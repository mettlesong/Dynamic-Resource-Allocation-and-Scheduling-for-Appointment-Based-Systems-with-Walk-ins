function [VF_DLP_heu_simu_new] = DLP_heu_simu_new_orig(N,m,r,w,pd0,pd1,pi,theta,DLP_allocation,DLP_scheduling,bar_k,p_s)
...
%generate random arrivals for all settings and all slots
    ...
    a=zeros(1,N);
    for t=1:N
        a(t)=sum(rand_scheduling(1:theta,t));
    end
    ...
    for j=2:N
        a_showup=randsrc(1,1,[0:m*theta;q_s(j,:)]);
        ...
        if j==2
            ...
            queue12(2,j)=queue12(2,j-1)+a_showup-DLP_Heuristic_allocation(2); %DLP_Heuristic_allocation(2):allocation of type-2 customers in period j-1
        ...
        VF=VF+r(1)*DLP_Heuristic_allocation(1)+r(2)*DLP_Heuristic_allocation(2)+((N-j)*w(1)+pi(1))*(DLP_Heuristic_allocation(1)-sum(rand_d1(1:theta,j-1)))+((N-j)*w(2)+pi(2))*(DLP_Heuristic_allocation(2)-a_showup);
    end
