function [DLP_bar_vf,DLP_allocation,DLP_scheduling,bar_k] = DLP(N,m,r,w,pd0,pd1,pi,theta,y)
...
% y_theta is the number of appointment customers who make appointemnts
...
for j=2:bar_k   %allocate inpatients first
    ...
    DLP_allocation(2,j)=min(max(m-mu_0-DLP_allocation(1,j),0),y-m-sum(DLP_allocation(2,2:j-1)));
    DLP_scheduling(j)=DLP_allocation(2,j);
end

for j=(1+bar_k):N   %allocate outpatients first
    %DLP_allocation(2,j)=m-mu_0; 
    DLP_allocation(2,j)=min(m-mu_0,y-m-sum(DLP_allocation(2,2:j-1)));
    DLP_scheduling(j)=DLP_allocation(2,j);
    %DLP_allocation(1,j)=0;
    DLP_allocation(1,j)=min(max(m-mu_0-DLP_allocation(2,j),0),(j-1)*mu_1-sum(DLP_allocation(1,2:j-1)));  
end
