function [vec_v_HS_simple,v_HS_star_simple,s_HS_star_index]=get_V_HS_simple_linear(N,m,r,w,pd0_theta,pd1_theta,pi,theta,y)
...
vec_v_HS_simple=zeros(1,length(y));
for index=1:length(y)
    ...
    vec_v_HS_simple(index)=V_1(1,1);
    if V_1(1,1)>=v_HS_star_simple  %%%find optimal value and schedule number. Note that the profit is calcuated from period 2 onwards.
        v_HS_star_simple=V_1(1,1);
        s_HS_star_index=index;     
    end
